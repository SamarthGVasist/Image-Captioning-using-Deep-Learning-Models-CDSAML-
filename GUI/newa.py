...
model = load_model("../models/4LSTM/model-StyleNew15000_3000-Batch128-Thresh10-Attention-0.5Dropout-1BiLSTM-End-relu-Adam-ep012-loss2.845-val_loss3.633.h5", custom_objects={'Attention': Attention})
...
def home():
    flash('Welcome to CDSAML', 'success')
...
# load and prepare the photograph
        ...
        if filename[:-3] in all_features:
            photo = all_features[filename[:-3]]
        else:
            photo = extract_features(destination)
        print("Time to extract features: ", timer()-start)
# generate description
        start = timer()
        description = generate_desc(model, tokenizer, photo, max_length, graph)
...
#         #return description
    ...
    # return send_from_directory("images", filename, as_attachment=True)
        ...
        return render_template("complete1.html", image_name=filename,caption=description)
...
def login():
    flash('Welcome to CAPSTYLE','success')
    form = LoginForm()
    print(form.email.data)

    if form.validate_on_submit():

        if form.email.data == 'admin@style.com' and form.password.data == 'capstyle':
